package com.itcpay.shiro.constroller;
...
public class LoginController {
    ...
    public String denied() {
        log.info("小伙子权限啊不足，别无谓挣扎了...");
        return "denied...";
    }

    @GetMapping("/login")
    public String login(String username, String password, RedirectAttributes model) {
        // 想要得到 SecurityUtils.getSubject()的对象...访问地址必须跟shiro的拦截地址内，不然会报空指针
        Subject sub = SecurityUtils.getSubject();
        // 用户输入的账号和密码，存储到UsernamePasswordToken对象中，然后由shiro内部认证对比
        // 认证执行者交由 com.itcpay.config.AuthRealm 中 doGetAuthenticationInfo 处理
